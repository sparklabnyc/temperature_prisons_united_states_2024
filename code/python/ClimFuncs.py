...
#
#    Clim Funcs
#    By Cascade Tuholske Spring 2021, updated Feb 2022
#
#    Functions needed for 02_WBGTMax.py ... See notes in that script for 
#    References.
#
...
def make_rh(tmax, vpdmax):
    """ Equation from Spangler et al 2018 to caluclate relative min humidity from Tmax and vpdmax 
    with the assumption that RHmin happens at Tmax during a diurnal period.
        Args:
            tmax = tmax from prism in °C
            vpdmax = vapor pressure deficit from prism data
    """
    
    # vpdmax from prism is in hpa so need to scale
    vpdmax = vpdmax * 100
    
    top = 610.94 * np.exp(17.625 * tmax / (243.04+tmax)) - vpdmax
    bottom = 610.94 * np.exp(17.625 * tmax / (243.04+tmax))
    rh = 100 * (top / bottom) 
