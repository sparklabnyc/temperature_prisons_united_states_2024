...
plot_state_year_difference_sorted = function(threshold_chosen,legend_chosen=0){
  ...
  dat.wbgt.summarised.merged.weighted.prison.state.sorted = dat.wbgt.summarised.merged.weighted.prison.state %>%
    group_by(STATE) %>%
    dplyr::mutate(mean = mean(get(paste0('wbgt_',threshold_chosen,'_diff')),na.rm = TRUE)) %>%
    dplyr::arrange((mean)) %>%
    ungroup() %>%
  ...
  if(legend_chosen==1){
    p = p + 
      guides(fill = guide_colorbar(direction = "horizontal", title.position="left",barwidth = 10, 
                                 barheight = 1,title.vjust = 0.8,
                                 title = "Population-weighted difference in annual\nnumber of hot-humid days between carceral\nfacilities and rest of state"),legend.text=element_text(size=15))
  }
...
plot.28 = plot_grid(plot.state.year.diff.28.sorted, nested.28,
          label_size = 14 ,
          label_x = -0.02, label_y = 1,
          labels = c('(a)'))
...
## 28°C: Place arrangement of plots (heatmap alphabetized)
```{r}
nested.28 = plot_grid(plot.total.change.28, plot.total.disparity.change.28,
          nrow = 2,
          label_size = 14 ,
...
plot.28.alpha.sorted = plot_grid(plot.state.year.diff.28, nested.28,
          label_size = 14 ,
          label_x = -0.02, label_y = 1,
          labels = c('(a)'))

pdf(paste0(wbgtmax.folder,'Figure_2_28_alpha_sorted.pdf'),paper='a4r',width=0,height=0)
plot.28.alpha.sorted
dev.off()
...
## Composite figure for research brief 
```{r}
plot.state.year.diff.28.sorted #rb figure 1b
plot.bar.chart.28 #rb figure 1a

rb_figure = plot_grid(plot.bar.chart.28, plot.state.year.diff.28.sorted,
          label_size = 14 ,
          label_x = -0.02, label_y = 1,
          labels = c('(a)','(b)'))

pdf(paste0(wbgtmax.folder,'research_brief_figure.pdf'),paper='a4r',width=-2,height=0)
rb_figure
dev.off()
```
