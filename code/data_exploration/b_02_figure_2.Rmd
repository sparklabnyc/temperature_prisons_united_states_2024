...
```{r}
nested.28 = plot_grid(plot.total.change.28, plot.total.disparity.change.28,
          nrow = 2,
          label_size = 14 ,
          label_x = -0.02, label_y = 1,
          labels = c('(b)', '(c)'))

plot.28 = plot_grid(plot.state.year.diff.28, nested.28,
          label_size = 14 ,
          label_x = -0.02, label_y = 1,
          labels = c('(a)'))
...
pdf(paste0(wbgtmax.folder,'Figure_2_30.pdf'),paper='a4r',width=0,height=0)
plot.30
dev.off()
...
## 26°C: Place arrangement of plots (heatmap arranged by average disparity)
```{r}
nested.26 = plot_grid(plot.total.change.26, plot.total.disparity.change.26,
          nrow = 2,
          label_size = 14 ,
...
plot.26.sorted = plot_grid(plot.state.year.diff.26.sorted, nested.26,
          label_size = 14 ,
          label_x = -0.02, label_y = 1,
          labels = c('(a)'))

pdf(paste0(wbgtmax.folder,'Figure_2_26_sorted.pdf'),paper='a4r',width=0,height=0)
plot.26.sorted
dev.off()
...
## 28°C: Place arrangement of plots (heatmap arranged by average disparity)
```{r}
nested.28 = plot_grid(plot.total.change.28, plot.total.disparity.change.28,
          nrow = 2,
          label_size = 14 ,
...
plot.28.sorted = plot_grid(plot.state.year.diff.28.sorted, nested.28,
          label_size = 14 ,
          label_x = -0.02, label_y = 1,
          labels = c('(a)'))

pdf(paste0(wbgtmax.folder,'Figure_2_28_sorted.pdf'),paper='a4r',width=0,height=0)
plot.28.sorted
dev.off()
...
## 30°C: Place arrangement of plots (heatmap arranged by average disparity)
```{r}
nested.30 = plot_grid(plot.total.change.30, plot.total.disparity.change.30,
          nrow = 2,
          label_size = 14 ,
...
plot.30.sorted = plot_grid(plot.state.year.diff.30.sorted, nested.30,
          label_size = 14 ,
          label_x = -0.02, label_y = 1,
          labels = c('(a)'))

pdf(paste0(wbgtmax.folder,'Figure_2_30_sorted.pdf'),paper='a4r',width=0,height=0)
plot.30.sorted
dev.off()
...
dat.wbgt.summarised.regression.disparity.output = dat.wbgt.summarised.regression %>%
  dplyr::select(prison_id,total_change_disparity_wbgt_28) %>%
  dplyr::mutate(total_change_disparity_wbgt_28=round(total_change_disparity_wbgt_28,1)) %>%
  left_join(shapefile.data,by=c('prison_id'='FID')) %>% 
  filter(STATUS=='OPEN') %>% filter(POPULATION > 0) %>%
  dplyr::select(prison_id,total_change_disparity_wbgt_28,NAME,STATE,STATEFP,COUNTY,CITY,TYPE,POPULATION,CAPACITY,VAL_DATE)

readr::write_csv(dat.wbgt.summarised.regression.disparity.output ,paste0(wbgtmax.folder,'Figure_2c.csv'))
```