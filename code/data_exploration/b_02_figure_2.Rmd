...
```{r}
legend_create_count_start_year = function(threshold)(paste0('Number of days\nabove threshold in ',start_year_wbgt))
legend_create_count_end_year = function(threshold)(paste0('Number of days\nabove threshold in ',end_year_wbgt))
legend_create_annual_change = function(threshold)(paste0('Annual change during\n',start_year_wbgt,'-',end_year_wbgt ))
legend_create_total_change = function(threshold)(paste0('Total change in annual number of days\nabove threshold during ',start_year_wbgt,'-',end_year_wbgt))
```
...
plot_start_year = function(threshold_chosen,legend_chosen=0){
  p = ggplot() +
    ...
    scale_color_gradientn(colors=(MetBrewer::met.brewer('OKeeffe2')), limits = c(0,max(USA.df.merged$wbgt_26_1982))) +
...
plot_end_year = function(threshold_chosen,legend_chosen=0){
  p = ggplot() +
    ...
    scale_color_gradientn(colors=rev(MetBrewer::met.brewer('OKeeffe2')), limits = c(0,max(USA.df.merged$wbgt_26_1982))) +
    coord_fixed() + xlab('') + ylab('') + theme_bw() +
    theme(panel.grid.major = element_blank(),
        axis.text.x = element_text(angle=90), axis.ticks.x=element_blank(),legend.text=element_text(size=5),
        panel.grid.minor = element_blank(), axis.line = element_line(colour = "black"),
...
plot_total_change = function(threshold_chosen,legend_chosen=0){
  p = ggplot() +
    ...
    scale_color_gradientn(colors=rev(MetBrewer::met.brewer('Cassatt2')), limits = c(-1*max(USA.df.merged$total_change_wbgt_26),max(USA.df.merged$total_change_wbgt_26))) +
    coord_fixed() + xlab('') + ylab('') + theme_bw() +
    theme(panel.grid.major = element_blank(),
        axis.text.x = element_text(angle=90), axis.ticks.x=element_blank(), legend.text=element_text(size=5),
        panel.grid.minor = element_blank(), axis.line = element_line(colour = "black"),
...
plot_state_year_difference = function(threshold_chosen,legend_chosen=0){
  p = ggplot() +
    ...
    guides(fill = guide_colorbar(direction = "horizontal", title.position="left",barwidth = 10, 
                                 ...
                                 title = "Difference in number of hot-humid days"),legend.text=element_text(size=15)) +
    xlab('Year') +
    ylab('State') +
    scale_fill_gradient2(low=met.brewer('Demuth')[10],mid='cornsilk',high=met.brewer('Demuth')[1],
                         na.value="cornsilk",guide = guide_legend(nrow = 1)) +
...
gridExtra::grid.arrange(as_grob(plot.state.year.diff.26),
                        as_grob(plot.count.end.year.26),
...
gridExtra::grid.arrange(as_grob(plot.state.year.diff.28),
                        as_grob(plot.count.end.year.28),
...
<!-- ```{r} -->
<!-- pdf(paste0(wbgtmax.folder,'Figure_2_old.pdf'),paper='a4r',width=0,height=0) -->
<!-- gridExtra::grid.arrange(as_grob(plot.count.start.year.26),as_grob(plot.count.start.year.28),as_grob(plot.count.start.year.30), -->
<!--                         plot.count.start.year.legend, -->
<!--                         as_grob(plot.total.change.26),as_grob(plot.total.change.28),as_grob(plot.total.change.30), -->
