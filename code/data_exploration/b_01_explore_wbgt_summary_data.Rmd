...
legend_create_person_days_total_change = function(threshold)(paste0('Total change in annual number of person-days over WBGT ', threshold ,'°C'))
```
...
## Obtain population-weighted WGBT change values
```{r}
USA.df.merged = USA.df.merged %>%
  mutate(total_change_person_days = POPULATION * get(paste0('total_change_wbgt_',threshold_chosen)))
```

## Plot total person-days change map 
```{r}
plot.total.person.days.change = ggplot() +
    geom_polygon(data=us_aea,aes(x=long,y=lat,group=group),fill='cornsilk',color='black',size=0.2) +
    geom_polygon(data=USA.df.merged,aes(x=long,y=lat,group=group,color=total_change_person_days),size=2) +
    guides(color = guide_colorbar(direction = "horizontal", title.position="left",barwidth = 20,barheight = 1,title.vjust = 0.8, 
                                  title = legend_create_person_days_total_change(threshold_chosen)),
...
pdf(paste0(wbgtmax.folder,'map_total_change_wbgtmax_person_days_over_',threshold_chosen,'.pdf'),paper='a4r',height=0,width=0)
print(plot.total.person.days.change)
dev.off()

plot.total.person.days.change
```