...
```{r}
legend_create_count_start_year = function(threshold)(paste0(start_year_wbgt))
legend_create_count_end_year = function(threshold)(paste0(end_year_wbgt))
legend_create_annual_change = function(threshold)(paste0('Annual change'))
legend_create_total_change = function(threshold)(paste0('Total change'))
```

## Only take open prisons and whose population we know
```{r}
USA.df.merged = USA.df.merged %>% 
  filter(STATUS=='OPEN') %>% filter(POPULATION > -999) 
```
...
plot_start_year = function(threshold_chosen,legend_chosen=0){
  ggplot() +
    ...
    geom_polygon(data=USA.df.merged,aes(x=long,y=lat,group=group,color=get(paste0('wbgt_',threshold_chosen,'_',start_year_wbgt))),size=2) +
    ...
    scale_color_gradientn(colors=rev(MetBrewer::met.brewer('Tiepolo')), limits = c(0,max(USA.df.merged$wbgt_25_1981))) +
...
plot.count.start.year.25 = plot_start_year(25)
plot.count.start.year.28 = plot_start_year(28)
plot.count.start.year.30 = plot_start_year(30)
```

## Plot count in end year map 
```{r}
plot_end_year = function(threshold_chosen){
  ggplot() +
    geom_polygon(data=us_aea,aes(x=long,y=lat,group=group),fill='cornsilk',color='black',size=0.2) +
    geom_polygon(data=USA.df.merged,aes(x=long,y=lat,group=group,color=get(paste0('wbgt_',threshold_chosen,'_',end_year_wbgt))),size=2) +
    guides(color = guide_colorbar(direction = "horizontal", title.position="left",barwidth = 10,barheight = 1,title.vjust = 0.8, 
...
```{r}
plot_total_change = function(threshold_chosen){
  ggplot() +
    geom_polygon(data=us_aea,aes(x=long,y=lat,group=group),fill='cornsilk',color='black',size=0.2) +
    ...
    scale_color_gradientn(colors=rev(MetBrewer::met.brewer('Nizami')), limits = c(-1*max(USA.df.merged$total_change_wbgt_25),max(USA.df.merged$total_change_wbgt_25))) +
    coord_fixed() + xlab('') + ylab('') + theme_bw() +
    theme(panel.grid.major = element_blank(),
        axis.text.x = element_text(angle=90), axis.ticks.x=element_blank(),legend.text=element_text(size=20),
        panel.grid.minor = element_blank(), axis.line = element_line(colour = "black"),
...
plot.total.change.25 = plot_total_change(25)
plot.total.change.28 = plot_total_change(28)
plot.total.change.30 = plot_total_change(30)
```

## Place on single gridded plot
```{r}
pdf(paste0(wbgtmax.folder,'Figure_1_in_progress.pdf'),paper='a4r',width=0,height=0)
gridExtra::grid.arrange(plot.count.start.year.25,plot.count.start.year.28,plot.count.start.year.30,
                        plot.total.change.25,plot.total.change.28,plot.total.change.30,
                        nrow=2)
dev.off()
